...
def test_profile_api_returns_random_friend_code_only_to_owner(tmp_path):
    ...
    updated = client.put("/api/users/me/profile", json={
        ...
        "profile_style": {
            ...
            "avatar_frame": "neon",
            "avatar_shape": "custom",
            "avatar_mask": "polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)",
        },
        "profile_public_info": [
    ...
    assert updated.get_json()["profile"]["profile_style"]["avatar_size"] == "600"
    assert updated.get_json()["profile"]["profile_style"]["avatar_frame"] == "neon"
    assert updated.get_json()["profile"]["profile_style"]["avatar_shape"] == "custom"
    assert updated.get_json()["profile"]["profile_style"]["avatar_mask"].startswith("polygon(")
    assert updated.get_json()["profile"]["profile_public_info"] == [
        {"label": "作品集", "value": "https://example.test/alice"},
        {"label": "社群", "value": "@alice"},
    ]
    assert public.get_json()["profile"]["profile_template"] == "creator"
    assert public.get_json()["profile"]["profile_style"]["avatar_size"] == "600"
    assert public.get_json()["profile"]["profile_style"]["avatar_shape"] == "custom"
