...
AI_AGENT_WRITE_TOOL_SPECS = {
    ...
    "write_launch_preflight_execute": {
        "label": "執行上線前檢查",
        "description": "執行選用的 requirements、log chain、AI audit scan 並整理建議與後續指令；預設只 dry-run，僅在 root 明確提供 GO_LIVE 時切換 production。",
        "method": "DIRECT",
        "path_params": {},
...
def register_ai_agent_routes(app, deps):
    ...
    def _launch_preflight_summary(requirements_payload, logs_payload, audit_payload):
        blockers = []
    ...
    def _launch_current_mode(payload):
        if not isinstance(payload, dict):
            return ""
        mode = payload.get("mode")
        if isinstance(mode, dict):
            return str(mode.get("current_mode") or mode.get("mode") or mode.get("name") or "").strip()
        return str(mode or payload.get("current_mode") or "").strip()

    def _append_launch_blocker(blockers, message):
        message = str(message or "").strip()
        if message and message not in blockers:
            blockers.append(message)

    def _launch_readiness_actions(requirements_payload, advisories, *, auto_switch, final_mode):
        actions = []
        requirements_payload = requirements_payload if isinstance(requirements_payload, dict) else {}
        missing = [str(item) for item in requirements_payload.get("missing") or []]
        failed = [str(item) for item in requirements_payload.get("failed") or []]
        if final_mode != "production" and not auto_switch:
            actions.append({
                "kind": "explicit_go_live_confirmation",
                "agent_can_execute": True,
                "required_arguments": {"auto_switch": True, "confirm": "GO_LIVE"},
                "reason": (
                    "preflight completed; its findings are advisory, while production switching "
                    "remains a separate explicit root action"
                ),
            })
        for report_type in dict.fromkeys([*missing, *failed]):
            command = (
                ...
                if report_type == "operational_campaign_24h"
                else f"python3 scripts/on_live_reports/{report_type}.py"
            )
            actions.append({
                "kind": "generate_or_refresh_report",
                "report_type": report_type,
                "command": command,
                "agent_can_execute": False,
                "reason": "production evidence must be generated by the isolated operator/Codex test environment",
            })
        if advisories and not any(item.get("kind") == "generate_or_refresh_report" for item in actions):
            actions.append({
                "kind": "review_advisories",
                "agent_can_execute": False,
                "reason": "review log-chain, AI audit, integrity, and report details; they do not block an explicit GO_LIVE switch",
            })
        return actions

    def _launch_operator_runbook():
        return [
            {
                "kind": "repository_gate",
                "command": "python3 scripts/prepush/runner.py --full --ci",
                "agent_can_execute": False,
                "artifact_policy": "temporary runtime only",
            },
            {
                "kind": "full_pytest",
                "command": "scripts/testing/pytest_in_tmp.sh -q tests",
                "agent_can_execute": False,
                "artifact_policy": "/tmp",
            },
            {
                "kind": "whole_site_production_gate",
                "command": "scripts/security/pentest/run_pentest.sh --target <OWNED_STAGING_URL> --only whole-site-production-gate --i-own-this-target",
                "required_env": ["WHOLE_SITE_ROOT_PASSWORD", "HACKME_RUNTIME_DIR"],
                "agent_can_execute": False,
                "artifact_policy": "selected isolated runtime",
            },
            {
                "kind": "twenty_four_hour_operational_campaign",
                "command": "python3 scripts/testing/operational_campaign_24h.py --campaign-root /tmp/hackme_web_campaign_24h_<RUN_ID> --duration-seconds 86400 --account-count 10 --round-ops 1000 --concurrency 32 --session-pool 20 --resource-interval 5",
                "required_env": [],
                "optional_env": ["HACKME_CAMPAIGN_COMFYUI_API_URL"],
                "agent_can_execute": False,
                "artifact_policy": "/tmp only",
                "required_active_test_seconds": 86400,
                "authorization_wait_included": False,
                "coverage": [
                    "long_video_upload_stream_hls_share",
                    "multi_account_synchronized_operations",
                    "trading_and_background_trading",
                    "pointschain_hft_incident_governance",
            ...
            {
                "kind": "human_signoff",
                "document": "docs/security/PRODUCTION_SIGNOFF_CHECKLIST.md",
                "agent_can_execute": False,
            },
        ]
    ...
    def _execute_launch_preflight(actor, args, settings):
        ...
        auto_switch = _parse_bool(args.get("auto_switch")) is True
        ...
        switch_confirm = str(args.get("confirm") or "").strip()
        if auto_switch and switch_confirm != "GO_LIVE":
            return 400, {
                "ok": False,
                "msg": "切換 production 必須由 root 明確提供 confirm=GO_LIVE；一般上線前檢查只執行 dry-run",
                "target_mode": "production",
                "auto_switch": True,
                "required_confirm": "GO_LIVE",
            }
        ...
        advisories = _launch_preflight_summary(req_payload, logs_payload, audit_payload)
        blockers = []
        if not _launch_step_ok(req_status, req_payload):
            _append_launch_blocker(advisories, f"requirements gate 呼叫失敗：HTTP {int(req_status or 0)}")
        if not _launch_step_ok(logs_status, logs_payload):
            _append_launch_blocker(advisories, f"server-mode log chain 呼叫失敗：HTTP {int(logs_status or 0)}")
        switch_payload = None
        switch_status = None
        if auto_switch:
            switch_status, switch_payload = _dispatch_internal_api("POST", "/api/root/server-mode/switch", {
                "mode": "production",
                "confirm": switch_confirm,
                "reason": reason,
            })
            steps.append({
            ...
            if not _launch_step_ok(switch_status, switch_payload):
                _append_launch_blocker(blockers, f"production 切換呼叫失敗：HTTP {int(switch_status or 0)}")
        else:
            steps.append({
                "name": "switch_production",
                "status": 0,
                ...
                "result": {
                    "ok": True,
                    "msg": "dry-run 完成；未收到明確 GO_LIVE 指示，因此未切換 production",
                },
            })

        final_status, final_mode_payload = _dispatch_internal_api("GET", "/api/root/server-mode", None)
        final_mode = _launch_current_mode(final_mode_payload)
        final_ok = _launch_step_ok(final_status, final_mode_payload)
        steps.append({
            "name": "final_mode_status",
            "status": final_status,
            "ok": final_ok and bool(final_mode),
            "result": final_mode_payload,
        })
        if not final_ok:
            _append_launch_blocker(blockers, f"最終 server mode 回讀失敗：HTTP {int(final_status or 0)}")
        elif not final_mode:
            _append_launch_blocker(blockers, "最終 server mode 回讀缺少 current_mode")
        elif auto_switch and switch_payload is not None and final_mode != "production":
            _append_launch_blocker(blockers, f"production 切換後回讀模式不一致：{final_mode}")
        completed = final_mode == "production" and not blockers
        preflight_passed = not advisories
        next_actions = _launch_readiness_actions(
            req_payload,
            advisories,
        ...
        return 200, {
            ...
            "ready_to_switch": not blockers and final_mode != "production",
            "target_mode": "production",
            "final_mode": final_mode,
            "auto_switch": bool(auto_switch),
            "dry_run": not bool(auto_switch),
            "blockers": blockers,
            "advisories": advisories,
            "next_actions": next_actions,
            "operator_runbook": _launch_operator_runbook(),
            "operator_boundary": {
                "agent_can_inspect_and_explain": True,
                "agent_can_generate_shell_evidence": False,
                "codex_handoff_supported": True,
                "production_switch_requires_explicit_go_live": True,
            },
            "steps": steps,
            "final_status": {
                "status": final_status,
            ...
            "msg": (
                "已成功切換 production"
                if completed and auto_switch
                else (
                    "上線前檢查完成；目前已在 production，本次未執行切換"
                    if not auto_switch and final_mode == "production"
                    else (
                    "上線前檢查完成；本次為 dry-run，未切換 production"
                    if not auto_switch
                    else "production 切換未完成；請依 blockers 處理後重試"
                    )
                )
            ),
        }
    ...
    def _safe_launch_preflight_payload(payload):
        """Keep launch decisions machine-readable while bounding bulky evidence."""
        if not isinstance(payload, dict):
            return _safe_tool_payload(payload)

        compact = {key: value for key, value in payload.items() if key != "steps"}
        compact_steps = []
        for raw_step in payload.get("steps") or []:
            ...
            step = {
                ...
                for key in ("name", "status", "ok", "skipped")
                if key in raw_step
            }
            ...
            if isinstance(result, dict):
                ...
                result_summary = {
                    key: result.get(key)
                    for key in ("ok", "msg", "current_mode")
                    if key in result
                }
                if name == "requirements_gate":
                    result_summary["missing"] = list(result.get("missing") or [])
                    result_summary["failed"] = list(result.get("failed") or [])
                    reports = result.get("reports") if isinstance(result.get("reports"), dict) else {}
                    result_summary["report_status"] = {
                        str(report_type): {
                            key: report.get(key)
                            for key in (
                                "pass",
                ...
                elif name == "log_chain_verify" and "chain" in result:
                    result_summary["chain"] = _safe_tool_payload(result.get("chain"), max_chars=4000)
                elif name == "ai_agent_audit_scan":
                    scan = result.get("scan") if isinstance(result.get("scan"), dict) else result
                    result_summary["summary"] = (
                        scan.get("summary") if isinstance(scan.get("summary"), dict) else {}
                    )
                elif name in {"switch_production", "final_mode_status"} and "mode" in result:
                    result_summary["mode"] = _safe_tool_payload(result.get("mode"), max_chars=4000)
                step["result"] = result_summary
            elif result is not None:
                step["result"] = _safe_tool_payload(result, max_chars=2000)
            compact_steps.append(step)
        compact["steps"] = compact_steps
        return _safe_tool_payload(compact, max_chars=64000)
    ...
    def ai_agent_write_tool_execute_route():
        ...
        high_risk_requires_write_mode = bool(
            spec.get("write") and execution.get("risk_level") == "high"
        )
        elevated = bool(
            spec.get("write")
            and _actor_is_super_admin(actor)
            and elevate_once
            and not high_risk_requires_write_mode
        )
        if spec.get("write") and not execution["allowed"] and not elevated:
            denial_reason = (
                "high_risk_requires_write_mode"
                if high_risk_requires_write_mode
                else "operation_mode_not_write"
            )
            _audit_agent_event("AI_AGENT_WRITE_TOOL", actor, success=False, detail=f"tool={tool_name},error={denial_reason},mode={public.get('operation_mode')}")
            return json_resp({
                "ok": False,
                "msg": "此 action 的風險層級不允許在目前模式執行；請使用 assist-safe action，或由 root 切換為 write 模式",
                "operation_mode": public.get("operation_mode"),
                "requires_elevation": _actor_is_super_admin(actor) and not high_risk_requires_write_mode,
                "requires_write_mode": high_risk_requires_write_mode,
        ...
        result_payload = (
            _safe_launch_preflight_payload(payload)
            if tool_name == "write_launch_preflight_execute"
            else _safe_tool_payload(payload)
        )
        return json_resp({
            "ok": ok,
            "tool": tool_name,
