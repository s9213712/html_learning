...
AI_AGENT_WRITE_TOOL_SPECS = {
    ...
    "write_launch_preflight_execute": {
        "label": "執行上線前檢查",
        "description": "執行 requirements、log chain、AI audit scan 並整理阻塞原因與後續指令；預設只 dry-run，僅在 root 明確提供 GO_LIVE 時切換 production。",
        "method": "DIRECT",
        "path_params": {},
...
def register_ai_agent_routes(app, deps):
    ...
    def _launch_readiness_actions(requirements_payload, blockers, *, auto_switch, final_mode):
        actions = []
        ...
        if blockers and not actions:
            actions.append({
                ...
                "agent_can_execute": False,
                "reason": "review log-chain, AI audit, integrity, and report details before retrying",
            })
        if not blockers and final_mode != "production" and not auto_switch:
            actions.append({
                "kind": "explicit_go_live_confirmation",
                "agent_can_execute": True,
                "required_arguments": {"auto_switch": True, "confirm": "GO_LIVE"},
                "reason": "preflight passed; production switching remains a separate explicit root action",
            })
        return actions
    ...
    def _execute_launch_preflight(actor, args, settings):
        ...
        if not _launch_step_ok(req_status, req_payload):
            _append_launch_blocker(blockers, f"requirements gate 呼叫失敗：HTTP {int(req_status or 0)}")
        if not _launch_step_ok(logs_status, logs_payload):
            _append_launch_blocker(blockers, f"server-mode log chain 呼叫失敗：HTTP {int(logs_status or 0)}")
        switch_payload = None
        switch_status = None
        ...
        return 200, {
            ...
            "ready_to_switch": preflight_passed and final_mode != "production",
            "target_mode": "production",
            "final_mode": final_mode,
            "auto_switch": bool(auto_switch),
            ...
            "msg": (
                ...
                if completed and auto_switch
                    ...
                    "上線前檢查通過；目前已在 production，本次未執行切換"
                    if preflight_passed and not auto_switch and final_mode == "production"
                    else (
                    "上線前檢查通過；本次為 dry-run，未切換 production"
                    if preflight_passed and not auto_switch
                    else "上線前檢查未通過；未切換 production，請依 blockers 修正後重跑"
                    )
                )
            ),
        }
