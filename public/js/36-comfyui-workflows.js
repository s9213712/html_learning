...
let comfyuiTemplateSdxlSkipRefiner = false;
let comfyuiTemplateGgufProfileState = { bundleId: "", profileId: "", variantId: "" };
let comfyuiWorkflowAccountGeneration = 0;
let comfyuiTemplateDetailLoadGeneration = 0;
let comfyuiWorkflowPresetsLoadGeneration = 0;
let comfyuiWorkflowEditorLoadGeneration = 0;
...
const COMFYUI_SDXL_REFINER_WORKFLOW_ID = "origin_sdxl_txt2img";
const COMFYUI_ANIMA_WORKFLOW_ID = "origin_anima_txt2img";
const COMFYUI_GGUF_WORKFLOW_ID = "origin_sdxl_gguf_txt2img";
const COMFYUI_SDXL_REFINER_NODE_IDS = new Set(["11", "12", "15", "16"]);
const COMFYUI_GGUF_PROFILE_MODEL_NODE_IDS = new Set(["4", "10", "11"]);
...
const COMFYUI_TEMPLATE_MODEL_PREFIX_STOPWORDS = new Set(["model", "diffusion", "unet", "checkpoint", "ckpt", "vae", "clip", "text", "encoder", "base"]);
...
function resetComfyuiWorkflowEditor({ keepStatus = false } = {}) {
  comfyuiWorkflowEditorLoadGeneration += 1;
...
function renderComfyuiWorkflowRunList(runs = []) {
  ...
  return `<div class="comfyui-workflow-run-list">${runs.map((run) => {
    ...
    const model = params.model || params.checkpoint || params.diffusion_model || "";
    const prompt = String(params.prompt || run?.prompt || "").trim();
    const summary = [
      model ? `模型 ${model}` : "",
      params.seed !== undefined && params.seed !== null ? `seed ${params.seed}` : "",
      params.steps ? `steps ${params.steps}` : "",
      params.cfg ? `CFG ${params.cfg}` : "",
      params.skip_refiner ? "跳過 refiner" : "",
    ...
    return `
      <div class="comfyui-workflow-run-item">
        ...
        ${prompt ? `<div class="drive-card-sub">${sanitize(prompt.slice(0, 120))}</div>` : ""}
        ${run.error ? `<div class="drive-card-sub">${sanitize(run.error)}</div>` : ""}
        ${runId > 0 ? `
          <div class="drive-file-actions" style="justify-content:flex-start;margin-top:.35rem;">
            <button class="btn btn-sm" type="button" data-comfyui-workflow-run-apply="${sanitize(String(runId))}">套回本次</button>
          </div>
        ` : ""}
      </div>
    `;
...
async function applyComfyuiWorkflowRunToForm(runId) {
  const numericRunId = Number(runId || 0);
  if (!Number.isFinite(numericRunId) || numericRunId <= 0) {
    setComfyuiMessage("這筆 workflow 執行紀錄缺少可套回 ID，請重新整理。", false);
    return;
  }
  if (typeof loadComfyuiHistory === "function") {
    await loadComfyuiHistory();
  }
  if (typeof applyComfyuiHistoryToForm !== "function") {
    setComfyuiMessage("目前前台缺少歷史套回功能，請重新整理頁面。", false);
    return;
  }
  await applyComfyuiHistoryToForm(`workflow-${numericRunId}`);
}

function comfyuiWorkflowDependencyLabel(item = {}) {
  const status = item?.dependency_status || null;
  if (!status) return "未檢查";
  if (status.available === true) return "已驗證";
  const missing = [];
  if (Array.isArray(status.missing_nodes) && status.missing_nodes.length) missing.push(`node ${status.missing_nodes.length}`);
  if (Array.isArray(status.missing_models) && status.missing_models.length) missing.push(`模型 ${status.missing_models.length}`);
  if (Array.isArray(status.missing_loras) && status.missing_loras.length) missing.push(`LoRA ${status.missing_loras.length}`);
  if (Array.isArray(status.missing_controlnets) && status.missing_controlnets.length) missing.push(`ControlNet ${status.missing_controlnets.length}`);
  return missing.length ? `缺 ${missing.join(" / ")}` : "缺依賴";
}

function comfyuiTemplateSelectGroups(payload = {}) {
  return [
    { label: "官方 workflow", items: Array.isArray(payload.official_presets) ? payload.official_presets : [] },
    { label: "我的 workflow", items: Array.isArray(payload.my_presets) ? payload.my_presets : [] },
    { label: "公開 workflow", items: Array.isArray(payload.shared_presets) ? payload.shared_presets : [] },
...
let comfyuiTemplateModelShowAllFields = {};
let comfyuiTemplateLastFocusedTextFieldId = "";
...
function resetComfyuiWorkflowAccountState() {
  comfyuiWorkflowAccountGeneration += 1;
  comfyuiTemplateDetailLoadGeneration += 1;
  comfyuiWorkflowPresetsLoadGeneration += 1;
  comfyuiWorkflowEditorLoadGeneration += 1;
  ...
  comfyuiTemplateSdxlSkipRefiner = false;
  comfyuiTemplateGgufProfileState = { bundleId: "", profileId: "", variantId: "" };
  comfyuiTemplateLoraOverrides = {};
  comfyuiTemplateFieldOverrides = {};
  comfyuiTemplateEditableModelFields = {};
  comfyuiTemplateModelShowAllFields = {};
  comfyuiTemplateLastFocusedTextFieldId = "";
  ...
  resetComfyuiWorkflowEditor();
  const select = $("comfyui-template-select");
  if (select) select.innerHTML = '<option value="">先選擇 workflow</option>';
  ["comfyui-template-summary", "comfyui-template-panels"].forEach((id) => {
    const el = $(id);
    if (el) el.replaceChildren();
  });
  const hint = $("comfyui-template-select-hint");
  if (hint) hint.textContent = "";
}

window.resetComfyuiWorkflowAccountState = resetComfyuiWorkflowAccountState;
...
function renderComfyuiTemplateSelector(payload = {}, { silentReload = true } = {}) {
  ...
  if (hint) {
    ...
    hint.textContent = total
      ? `生圖分頁列出可讀 workflow，共 ${total} 個；缺依賴項目會在名稱後標示，匯入、編輯、複製與發布請到 Workflow 分頁。`
      : "目前沒有可讀 workflow；請到 Workflow 分頁匯入或檢查官方模板。";
  }
  const options = ['<option value="">先選擇 workflow</option>'].concat(groups.map((group) => `
    <optgroup label="${sanitize(group.label)}">
      ${group.items.map((item) => `
        <option value="${sanitize(String(item.id))}">
          ${sanitize(item.title || `Workflow #${item.id}`)} — ${sanitize(comfyuiWorkflowDependencyLabel(item))}
        </option>
      `).join("")}
    </optgroup>
  `));
  ...
  if (select.dataset.comfyuiTemplateBound !== "1") {
    ...
    select.addEventListener("change", () => {
      ...
      loadComfyuiSelectedTemplateDetail(presetId, { silent: false }).catch((err) => {
        if (err?.name !== "AbortError") setComfyuiMessage(err.message || "模板讀取失敗", false);
      });
    });
  }
  if (comfyuiSelectedTemplatePresetId) {
    ...
    if (silentReload && isSameDetail) {
      ...
      loadComfyuiSelectedTemplateDetail(comfyuiSelectedTemplatePresetId, {
        ...
        if (err?.name !== "AbortError") reportFrontendFailure("comfyui-template-silent-reload", err);
      });
    }
  } else {
    if (typeof updateComfyuiPreviewCardForOutputKinds === "function") updateComfyuiPreviewCardForOutputKinds(["image"]);
...
async function loadComfyuiSelectedTemplateDetail(presetId, { silent = false, applyDefaults = true, operation = null } = {}) {
  if (!presetId) return;
  const operationContext = operation || comfyuiOperationContext();
  const loadGeneration = ++comfyuiTemplateDetailLoadGeneration;
  comfyuiAssertOperationCurrent(operationContext);
  ...
  comfyuiAssertOperationCurrent(operationContext);
  if (loadGeneration !== comfyuiTemplateDetailLoadGeneration) throw comfyuiAccountAbortError();
  ...
  resetComfyuiSdxlRefinerState(comfyuiSelectedTemplateDetail);
...
async function runSelectedComfyuiWorkflowTemplateFromGenerate(mode, options = {}) {
  const operation = options.operation || comfyuiOperationContext();
  const runToken = options.runToken || null;
  comfyuiAssertOperationCurrent(operation);
  ...
  if (!comfyuiWorkflowPresets.length && typeof loadComfyuiWorkflowPresets === "function") {
    await loadComfyuiWorkflowPresets({ silentTemplateReload: true, operation });
    if (runToken) comfyuiAssertGenerationCurrent(operation, runToken);
    else comfyuiAssertOperationCurrent(operation);
  }
  ...
  if (Number(comfyuiSelectedTemplateDetail?.id || 0) !== Number(presetId)) {
    await loadComfyuiSelectedTemplateDetail(presetId, { silent: !autoSelected, applyDefaults: autoSelected, operation });
    if (runToken) comfyuiAssertGenerationCurrent(operation, runToken);
    else comfyuiAssertOperationCurrent(operation);
  }
  ...
  await runComfyuiWorkflowPreset(presetId, { operation, runToken });
  return true;
}
...
function comfyuiTemplateIsSdxlRefinerWorkflow(detail = comfyuiSelectedTemplateDetail) {
  return comfyuiTemplateBundleId(detail) === COMFYUI_SDXL_REFINER_WORKFLOW_ID
    || String(detail?.title || detail?.name || "").trim() === "SDXL Text-to-Image";
}

function comfyuiTemplateIsGgufWorkflow(detail = comfyuiSelectedTemplateDetail) {
  return comfyuiTemplateBundleId(detail) === COMFYUI_GGUF_WORKFLOW_ID
    || String(detail?.title || detail?.name || "").trim() === "SDXL GGUF Text-to-Image (ComfyUI-GGUF)";
}

function comfyuiTemplateSupportsEmbeddingShortcuts(detail = comfyuiSelectedTemplateDetail) {
  const bundleId = comfyuiTemplateBundleId(detail).toLowerCase();
  const title = String(detail?.title || detail?.name || "").toLowerCase();
  return bundleId !== COMFYUI_ANIMA_WORKFLOW_ID && !bundleId.includes("anima") && !title.includes("anima");
}

function comfyuiEnabledGgufProfiles() {
  const profiles = Array.isArray(comfyuiGgufProfiles) ? comfyuiGgufProfiles : [];
  return profiles.filter((profile) => profile && profile.id && profile.enabled !== false);
}

function comfyuiGgufProfileById(profileId = "") {
  return comfyuiEnabledGgufProfiles().find((profile) => String(profile.id || "") === String(profileId || "")) || null;
}

function comfyuiEnabledGgufVariants(profile) {
  return (Array.isArray(profile?.variants) ? profile.variants : []).filter((variant) => variant && variant.id && variant.enabled !== false);
}

function comfyuiGgufVariantById(profile, variantId = "") {
  return comfyuiEnabledGgufVariants(profile).find((variant) => String(variant.id || "") === String(variantId || "")) || null;
}

function resetComfyuiGgufWorkflowState(detail = comfyuiSelectedTemplateDetail) {
  const bundleId = comfyuiTemplateBundleId(detail);
  if (!comfyuiTemplateIsGgufWorkflow(detail)) {
    comfyuiTemplateGgufProfileState = { bundleId: "", profileId: "", variantId: "" };
    return;
  }
  const profiles = comfyuiEnabledGgufProfiles();
  const defaultProfileId = String(detail?.default_params?.gguf_profile || "").trim();
  const profile = comfyuiGgufProfileById(defaultProfileId) || profiles[0] || null;
  const defaultVariantId = String(detail?.default_params?.gguf_variant || "").trim();
  const variants = comfyuiEnabledGgufVariants(profile);
  const variant = comfyuiGgufVariantById(profile, defaultVariantId) || variants[0] || null;
  comfyuiTemplateGgufProfileState = {
    bundleId,
    profileId: profile?.id || "",
    variantId: variant?.id || "",
  };
  applyComfyuiGgufWorkflowProfileDefaults(detail, { overwrite: false });
}

function ensureComfyuiGgufWorkflowState(detail = comfyuiSelectedTemplateDetail) {
  const bundleId = comfyuiTemplateBundleId(detail);
  if (!comfyuiTemplateIsGgufWorkflow(detail)) return null;
  if (comfyuiTemplateGgufProfileState.bundleId !== bundleId) resetComfyuiGgufWorkflowState(detail);
  return comfyuiTemplateGgufProfileState;
}

function comfyuiGgufWorkflowRunSpec(detail = comfyuiSelectedTemplateDetail) {
  const state = ensureComfyuiGgufWorkflowState(detail);
  if (!state || !state.profileId || !state.variantId) return null;
  return {
    enabled: true,
...
function applyComfyuiGgufWorkflowProfileDefaults(detail = comfyuiSelectedTemplateDetail, { overwrite = false } = {}) {
  const state = ensureComfyuiGgufWorkflowState(detail);
  const profile = comfyuiGgufProfileById(state?.profileId || "");
  const defaults = profile?.sampler_defaults && typeof profile.sampler_defaults === "object" ? profile.sampler_defaults : {};
  if (!profile || !Object.keys(defaults).length) return;
  const supported = new Set(["sampler_name", "scheduler", "cfg", "steps"]);
  comfyuiTemplateAllFields(detail).forEach((field) => {
    if (String(field?.node_id || "") !== "3" || !supported.has(String(field?.input_name || ""))) return;
    const value = defaults[field.input_name];
    if (value === undefined || value === null) return;
    const key = comfyuiTemplateOverrideKey(field);
    if (!key) return;
    if (overwrite || !Object.prototype.hasOwnProperty.call(comfyuiTemplateFieldOverrides, key)) {
      comfyuiTemplateFieldOverrides[key] = value;
    }
  });
}

function resetComfyuiSdxlRefinerState(detail = comfyuiSelectedTemplateDetail) {
  comfyuiTemplateSdxlSkipRefiner = comfyuiTemplateIsSdxlRefinerWorkflow(detail)
    ? !!detail?.default_params?.skip_refiner
    : false;
  resetComfyuiGgufWorkflowState(detail);
}

function comfyuiSdxlRefinerRunSpec(detail = comfyuiSelectedTemplateDetail) {
  if (!comfyuiTemplateIsSdxlRefinerWorkflow(detail)) return null;
  return {
    enabled: true,
    skip_refiner: !!comfyuiTemplateSdxlSkipRefiner,
  };
}
...
function comfyuiMultiCompareCheckpointOptions(detail = comfyuiSelectedTemplateDetail) {
  const checkpointField = comfyuiTemplateCheckpointFields(detail)[0] || {};
  return comfyuiTemplateModelSelectOptions(checkpointField);
}
...
function comfyuiTemplateSharedPromptSourceField(detail, field = {}) {
  ...
  const visibleFields = fields.filter((candidate) => !comfyuiTemplateIsHiddenSdxlRefinerField(detail, candidate));
  return visibleFields[0] || fields[0] || null;
}
...
function comfyuiTemplateIsHiddenSdxlRefinerField(detail, field = {}) {
  if (!comfyuiTemplateIsSdxlRefinerWorkflow(detail) || !comfyuiTemplateSdxlSkipRefiner) return false;
  return COMFYUI_SDXL_REFINER_NODE_IDS.has(String(field?.node_id || ""));
}

function comfyuiTemplateIsHiddenGgufProfileField(detail, field = {}) {
  const state = ensureComfyuiGgufWorkflowState(detail);
  if (!state || !state.profileId) return false;
  return COMFYUI_GGUF_PROFILE_MODEL_NODE_IDS.has(String(field?.node_id || ""))
    && String(field?.category || "").toUpperCase() === "MODEL";
}
...
function comfyuiTemplateIsHiddenField(detail, field = {}) {
  return comfyuiTemplateIsHiddenSharedPromptField(detail, field)
    ...
    || comfyuiTemplateIsHiddenSdxlRefinerField(detail, field)
    || comfyuiTemplateIsHiddenGgufProfileField(detail, field)
...
function comfyuiTemplateSelectValuesFromDom(targetId = "") {
  const target = $(targetId);
...
function comfyuiTemplateModelOptionKind(field = {}) {
  const classType = String(field?.class_type || "");
  const inputName = String(field?.input_name || "");
  if (classType === "CheckpointLoaderSimple" && inputName === "ckpt_name") return "checkpoints";
  if (["LTXVAudioVAELoader", "LTXAVTextEncoderLoader"].includes(classType) && inputName === "ckpt_name") return "checkpoints";
  if (classType === "VAELoader" && inputName === "vae_name") return "vaes";
  if (
    (classType === "CLIPLoader" && inputName === "clip_name") ||
    (classType === "DualCLIPLoader" && ["clip_name1", "clip_name2"].includes(inputName)) ||
    (classType === "TripleCLIPLoader" && ["clip_name1", "clip_name2", "clip_name3"].includes(inputName)) ||
    (classType === "CLIPLoaderGGUF" && inputName === "clip_name") ||
    (classType === "DualCLIPLoaderGGUF" && ["clip_name1", "clip_name2"].includes(inputName)) ||
    (classType === "TripleCLIPLoaderGGUF" && ["clip_name1", "clip_name2", "clip_name3"].includes(inputName))
  ) return "clip_models";
  if (classType === "CLIPVisionLoader" && inputName === "clip_name") return "clip_vision_models";
  if (["UNETLoader", "UnetLoaderGGUF", "UnetLoaderGGUFAdvanced"].includes(classType) && inputName === "unet_name") return "diffusion_models";
  if (classType === "UpscaleModelLoader" && inputName === "model_name") return "upscale_models";
  if (classType === "LatentUpscaleModelLoader" && inputName === "model_name") return "latent_upscale_models";
  if (classType === "ControlNetLoader" && inputName === "control_net_name") return "controlnet_models";
  return "";
}

function comfyuiTemplateModelOptionKindLabel(kind = "") {
  return {
    checkpoints: "Checkpoint / 大模型",
    vaes: "VAE",
    clip_models: "CLIP / 文字編碼器",
    clip_vision_models: "CLIP Vision 模型",
    diffusion_models: "Diffusion / UNet 大模型",
    upscale_models: "Upscale 模型",
    latent_upscale_models: "Latent Upscale 模型",
    controlnet_models: "ControlNet 模型",
  }[String(kind || "")] || "模型";
}

function comfyuiTemplateModelCatalogValues(kind = "") {
  if (kind === "checkpoints") {
    return (typeof comfyuiAvailableCheckpoints !== "undefined" && Array.isArray(comfyuiAvailableCheckpoints) && comfyuiAvailableCheckpoints.length)
      ? comfyuiAvailableCheckpoints
      : comfyuiTemplateSelectValuesFromDom("comfyui-model-select");
  }
  if (kind === "vaes") return (typeof comfyuiAvailableVaes !== "undefined" && Array.isArray(comfyuiAvailableVaes)) ? comfyuiAvailableVaes : [];
  if (kind === "clip_models") return (typeof comfyuiAvailableClipModels !== "undefined" && Array.isArray(comfyuiAvailableClipModels)) ? comfyuiAvailableClipModels : [];
  if (kind === "clip_vision_models") return (typeof comfyuiAvailableClipVisionModels !== "undefined" && Array.isArray(comfyuiAvailableClipVisionModels)) ? comfyuiAvailableClipVisionModels : [];
  if (kind === "diffusion_models") return (typeof comfyuiAvailableDiffusionModels !== "undefined" && Array.isArray(comfyuiAvailableDiffusionModels)) ? comfyuiAvailableDiffusionModels : [];
  if (kind === "upscale_models") return (typeof comfyuiUpscaleModels !== "undefined" && Array.isArray(comfyuiUpscaleModels)) ? comfyuiUpscaleModels : [];
  if (kind === "latent_upscale_models") return (typeof comfyuiAvailableLatentUpscaleModels !== "undefined" && Array.isArray(comfyuiAvailableLatentUpscaleModels)) ? comfyuiAvailableLatentUpscaleModels : [];
  if (kind === "controlnet_models") {
    return (typeof comfyuiAvailableControlnetModels !== "undefined" && Array.isArray(comfyuiAvailableControlnetModels) && comfyuiAvailableControlnetModels.length)
      ? comfyuiAvailableControlnetModels
      : comfyuiTemplateSelectValuesFromDom("comfyui-controlnet-model");
  }
  return [];
}

function comfyuiTemplateUniqueOptions(values = []) {
  const seen = new Set();
  const options = [];
...
function comfyuiTemplateModelBasename(value = "") {
  return String(value || "").trim().replace(/\\/g, "/").split("/").pop().toLowerCase();
}

function comfyuiTemplateResolveModelOption(value = "", options = []) {
  const wanted = String(value || "").trim();
  if (!wanted) return "";
  const values = comfyuiTemplateUniqueOptions(options);
  if (values.includes(wanted)) return wanted;
  const normalizedWanted = wanted.replace(/\\/g, "/");
  const pathMatch = values.find((option) => option.replace(/\\/g, "/") === normalizedWanted);
  if (pathMatch) return pathMatch;
  const wantedBase = comfyuiTemplateModelBasename(wanted);
  const matches = values.filter((option) => comfyuiTemplateModelBasename(option) === wantedBase);
  return matches.length === 1 ? matches[0] : "";
}

function comfyuiTemplateModelFamilyPrefix(value = "") {
  const base = comfyuiTemplateModelBasename(value).replace(/\.(safetensors|ckpt|pt|pth|bin|gguf)$/i, "");
  const token = (base.split(/[-_.\s]+/).find((part) => part && part.length >= 3) || "").toLowerCase();
  return token && !COMFYUI_TEMPLATE_MODEL_PREFIX_STOPWORDS.has(token) ? token : "";
}

function comfyuiTemplateModelSelectFilterState(field = {}) {
  const kind = comfyuiTemplateModelOptionKind(field);
  const label = comfyuiTemplateModelOptionKindLabel(kind);
  const catalog = comfyuiTemplateUniqueOptions(comfyuiTemplateModelCatalogValues(kind));
  const currentRaw = String(comfyuiTemplateEffectiveFieldValue(field) ?? "").trim();
  const prefix = comfyuiTemplateModelFamilyPrefix(currentRaw);
  const prefixMatches = prefix
    ? catalog.filter((option) => comfyuiTemplateModelBasename(option).includes(prefix))
    : [];
  const showAll = !!comfyuiTemplateModelShowAllFields[String(field?.id || "")];
  const filtered = !!(!showAll && prefix && prefixMatches.length && prefixMatches.length < catalog.length);
  return { kind, label, catalog, currentRaw, prefix, prefixMatches, showAll, filtered };
}

function comfyuiTemplateModelSelectOptions(field = {}) {
  const { catalog, currentRaw, prefixMatches, filtered } = comfyuiTemplateModelSelectFilterState(field);
  if (!catalog.length) {
    return currentRaw
      ? [{ value: currentRaw, label: `${currentRaw}（目前未列出，仍會送出）`, disabled: false }]
      : [];
  }
  let options = filtered ? prefixMatches : catalog;
  const resolvedCurrent = comfyuiTemplateResolveModelOption(currentRaw, catalog);
  if (resolvedCurrent && !options.includes(resolvedCurrent)) options = [resolvedCurrent].concat(options);
  if (currentRaw && !resolvedCurrent && !options.includes(currentRaw)) options = [currentRaw].concat(options);
  return comfyuiTemplateUniqueOptions(options).map((value) => ({
    value,
    label: value === currentRaw && !resolvedCurrent ? `${value}（目前未列出，仍會送出）` : value,
    disabled: false,
  }));
}

function comfyuiTemplateModelSelectCurrentValue(field = {}, options = []) {
  const current = String(comfyuiTemplateEffectiveFieldValue(field) ?? "").trim();
  if (!current) return "";
  const resolved = comfyuiTemplateResolveModelOption(current, options.map((option) => option.value));
  return resolved || current;
}

function comfyuiTemplateModelSelectHint(field = {}, options = []) {
  const { label, catalog, currentRaw: current, prefix, prefixMatches, showAll, filtered } = comfyuiTemplateModelSelectFilterState(field);
  if (!catalog.length) {
    return current
      ? `目前 ComfyUI 沒有列出可用的 ${label}；會先以「${current}」送出，若後端找不到會回報錯誤。`
      : `目前 ComfyUI 沒有列出可用的 ${label}，請先安裝或同步模型後重新整理模型清單。`;
  }
  const resolved = comfyuiTemplateResolveModelOption(current, catalog);
  if (current && !resolved) return `目前 ComfyUI ${label} 清單未列出「${current}」；會先以這個值送出，若後端找不到會回報錯誤。`;
  if (filtered) return `已依模板預設前綴「${prefix}」篩出 ${prefixMatches.length} 個 ${label} 候選。按下方「重設篩選：顯示全部候選」可不要依前綴篩選，改看全部 ${catalog.length} 個候選。`;
  if (showAll && prefix && prefixMatches.length && prefixMatches.length < catalog.length) return `已顯示全部 ${catalog.length} 個 ${label} 候選；可恢復前綴「${prefix}」篩選。`;
  return `從目前 ComfyUI 已列出的 ${label} 中選擇。`;
}

function comfyuiTemplateModelSelectFilterActions(field = {}) {
  const { catalog, prefix, prefixMatches, showAll } = comfyuiTemplateModelSelectFilterState(field);
  if (!prefix || !prefixMatches.length || prefixMatches.length >= catalog.length) return "";
  const fieldId = sanitize(field?.id || "");
  const action = showAll ? "prefix" : "all";
  const label = showAll ? "恢復前綴篩選" : "重設篩選：顯示全部候選";
  const title = showAll ? `恢復只顯示前綴「${prefix}」的候選` : "不要依模板前綴篩選，顯示全部候選模型";
  return `
    <div class="drive-file-actions" style="justify-content:flex-start;margin-top:.45rem;">
      <button class="btn btn-sm" type="button" data-comfyui-template-model-filter="${fieldId}" data-comfyui-template-model-filter-mode="${sanitize(action)}" title="${sanitize(title)}" aria-label="${sanitize(title)}">${sanitize(label)}</button>
    </div>
  `;
}
...
function comfyuiTemplateFieldBinding(field, detail, ctx) {
  ...
  const modelOptionKind = comfyuiTemplateModelOptionKind(field);
  ...
  if (comfyuiTemplateCanEditLockedModelField(field)) {
    return { kind: "model_select", fieldId: field.id, optionKind: modelOptionKind, editableLockedModel: true };
  }
  if (
    ...
    (["LTXVAudioVAELoader", "LTXAVTextEncoderLoader"].includes(field?.class_type) && field?.input_name === "ckpt_name")
  ) return { kind: "model_select", fieldId: field.id, optionKind: modelOptionKind, editableLockedModel: comfyuiTemplateCanEditLockedModelField(field) };
  if (field?.class_type === "CheckpointLoaderSimple" && field?.input_name === "ckpt_name") {
    return { kind: "model_select", fieldId: field.id, optionKind: modelOptionKind, editableLockedModel: false };
  }
  ...
  if (field?.class_type === "UpscaleModelLoader" && field?.input_name === "model_name") {
    return { kind: "model_select", fieldId: field.id, optionKind: modelOptionKind, editableLockedModel: false };
  }
  if (field?.class_type === "ControlNetLoader" && field?.input_name === "control_net_name") {
    return { kind: "model_select", fieldId: field.id, optionKind: modelOptionKind, editableLockedModel: false, enableControlnet: true };
  }
  ...
  if (String(field?.category || "").toUpperCase() === "MODEL") {
    return modelOptionKind
      ? { kind: "model_select", fieldId: field.id, optionKind: modelOptionKind, editableLockedModel: comfyuiTemplateCanEditLockedModelField(field) }
      : { kind: "readonly" };
  }
...
function comfyuiTemplateFieldLabel(field = {}, binding = null) {
  ...
  if (classType === "LTXVAudioVAELoader" && inputName === "ckpt_name") return "LTX Audio VAE Checkpoint";
  if (classType === "LTXAVTextEncoderLoader" && inputName === "ckpt_name") return "LTX Text Encoder Checkpoint";
...
function comfyuiTemplateFieldValue(binding, field = {}) {
  ...
  if (binding.kind === "model_select") {
    const el = $(`tmpl-${field.id || ""}`);
    if (el) return el.value || "";
    const options = comfyuiTemplateModelSelectOptions(field);
    return comfyuiTemplateModelSelectCurrentValue(field, options) || "";
  }
  return field?.current_value;
}
...
function comfyuiTemplateRuntimeValue(binding, field = {}) {
  ...
  if (binding.kind === "lora") {
    const el = $(`tmpl-${field.id || ""}`);
    if (el) {
      return typeof normalizeComfyuiLoraName === "function"
        ? normalizeComfyuiLoraName(el.value)
...
function comfyuiTemplateDirectHint(field = {}) {
  ...
  const inputName = String(field?.input_name || "");
  if (category === "MODEL") {
    if (classType === "VAELoader" && inputName === "vae_name") return "請從 ComfyUI 已列出的 VAE 候選中選擇；若清單為空，請先安裝或同步 VAE 後重新整理模型。";
    if (classType === "CLIPVisionLoader") return "請從 ComfyUI 已列出的 CLIP Vision 候選中選擇。";
    if (classType === "LatentUpscaleModelLoader") return "請從 ComfyUI 已列出的 Latent Upscale 候選中選擇；子資料夾模型會以 ComfyUI 回傳名稱顯示。";
    if (/clip/i.test(inputName)) return "請從 ComfyUI 已列出的 CLIP / text encoder 候選中選擇。";
    if (/unet/i.test(inputName)) return "請從 ComfyUI 已列出的 diffusion / UNet 候選中選擇，並對應 Flux、SD3.5、Wan、ANIMA 等模型種類。";
    return "請從 ComfyUI 已列出的模型候選中選擇；缺檔時相容性檢查會提示。";
  }
...
function comfyuiGgufProfileVariantForSnapshot(workflowJson = {}, payload = {}) {
  const explicitProfileId = String(payload?.gguf_profile || payload?.diffusers_gguf_profile || "").trim();
  const explicitVariantId = String(payload?.gguf_variant || payload?.diffusers_gguf_variant || "").trim();
  const explicitProfile = explicitProfileId ? comfyuiGgufProfileById(explicitProfileId) : null;
  const explicitVariant = explicitProfile ? comfyuiGgufVariantById(explicitProfile, explicitVariantId) : null;
  if (explicitProfile && (explicitVariant || !explicitVariantId)) {
    return { profile: explicitProfile, variant: explicitVariant || comfyuiEnabledGgufVariants(explicitProfile)[0] || null };
  }
  const node = workflowJson && typeof workflowJson === "object" ? workflowJson["4"] : null;
  const inputs = node && typeof node.inputs === "object" ? node.inputs : {};
  const ggufFile = String(payload?.diffusion_model || payload?.model || payload?.diffusers_gguf_file || inputs.unet_name || inputs.ckpt_name || "").trim();
  if (!ggufFile) return { profile: null, variant: null };
  for (const profile of comfyuiEnabledGgufProfiles()) {
    const variant = comfyuiEnabledGgufVariants(profile).find((item) => String(item?.gguf_file || item?.filename || "") === ggufFile);
    if (variant) return { profile, variant };
  }
  return { profile: null, variant: null };
}

function applyComfyuiTemplateHistorySnapshotToForm(workflowJson = {}, payload = {}) {
  ...
  const snapshotValueForField = (field) => {
    const node = workflowJson[String(field?.node_id || "")];
    const inputs = node && typeof node.inputs === "object" ? node.inputs : null;
    if (!inputs || !Object.prototype.hasOwnProperty.call(inputs, String(field?.input_name || ""))) return undefined;
  ...
  const payloadValueForBinding = (binding, field) => {
    ...
    if (targetId === "comfyui-sampler") {
      return typeof comfyuiResolvedSamplerValue === "function"
        ? (comfyuiResolvedSamplerValue(payload.sampler_name || "") || payload.sampler_name)
        : payload.sampler_name;
    }
    if (targetId === "comfyui-scheduler") {
      return typeof comfyuiResolvedSchedulerValue === "function"
        ? (comfyuiResolvedSchedulerValue(payload.scheduler || "", payload.sampler_name || "") || payload.scheduler || "normal")
        : payload.scheduler;
    }
  ...
  comfyuiTemplateAllFields(detail).forEach((field) => {
    ...
    const value = snapshotValueForField(field);
  ...
  comfyuiTemplateAllFields(detail).forEach((field) => {
    ...
    if (binding.kind === "lora") {
      comfyuiTemplateLoraOverrides[String(binding.nodeId || "")] = value;
    } else if (binding.kind === "lora_strength") {
      const selected = comfyuiSelectedLoraForTemplateNode(binding.nodeId);
      if (selected) selected[binding.strengthField] = Number.isFinite(Number(value)) ? Number(value) : value;
    } else {
      setComfyuiTemplateFieldOverride(field, value);
    }
    changed = true;
  });
  if (changed) {
    if (comfyuiTemplatePromptSharingMode(detail) === "shared") {
      ["positive", "negative"].forEach((role) => {
        let sharedValue;
        if (role === "positive" && Object.prototype.hasOwnProperty.call(payload || {}, "prompt")) {
          sharedValue = payload.prompt;
        } else if (role === "negative" && Object.prototype.hasOwnProperty.call(payload || {}, "negative_prompt")) {
          sharedValue = payload.negative_prompt;
        } else {
          const fields = comfyuiTemplatePromptFieldsByRole(detail)[role] || [];
          const snapshotField = fields.find((field) => {
            const value = snapshotValueForField(field);
            return value !== undefined && String(value ?? "").trim();
          });
          sharedValue = snapshotField ? snapshotValueForField(snapshotField) : comfyuiTemplateSharedPromptValue(role, detail);
        }
        syncComfyuiTemplateSharedPromptFields(role, sharedValue);
      });
    }
    renderSelectedComfyuiTemplate({ preserveOpenPanels: true });
  }
...
function comfyuiTemplateSyntheticVaeFieldValue(detail = comfyuiSelectedTemplateDetail) {
  if (!detail?.ui_schema?.panels) return "";
  const fields = comfyuiTemplateAllFields(detail);
  const field = fields.find((item) => (
    !!item?.synthetic && String(item.class_type || "") === "VAELoader" && String(item.input_name || "") === "vae_name"
  ));
  if (!field?.id) return "";
  const element = $(`tmpl-${field.id}`);
...
function comfyuiMultiCompareRunSpec(detail = comfyuiSelectedTemplateDetail) {
  ...
  const renderedCheckpointValues = Array.from(document.querySelectorAll("[data-comfyui-multi-compare-checkpoint]"))
    .sort((a, b) => Number(a.getAttribute("data-comfyui-multi-compare-checkpoint")) - Number(b.getAttribute("data-comfyui-multi-compare-checkpoint")))
    .map((select) => String(select.value || "").trim());
  const optionValues = new Set(comfyuiMultiCompareCheckpointOptions(detail).map((option) => option.value));
  const sourceCheckpoints = renderedCheckpointValues.length ? renderedCheckpointValues : state.checkpoints;
  ...
  const loraOptionValues = new Set(loraOptions.map((option) => option.value).filter(Boolean));
  const renderedLoraValues = Array.from(document.querySelectorAll("[data-comfyui-multi-compare-lora]"))
    .sort((a, b) => Number(a.getAttribute("data-comfyui-multi-compare-lora")) - Number(b.getAttribute("data-comfyui-multi-compare-lora")))
    .map((select) => String(select.value || "").trim());
  const sourceLoras = renderedLoraValues.length
    ? renderedLoraValues.map((name, index) => ({
      name,
      strength_model: document.querySelector(`[data-comfyui-multi-compare-lora-strength-model="${index}"]`)?.value,
      strength_clip: document.querySelector(`[data-comfyui-multi-compare-lora-strength-clip="${index}"]`)?.value,
    }))
    : state.loras;
  const loras = sourceLoras
    .map((item) => {
      const name = typeof normalizeComfyuiLoraName === "function" ? normalizeComfyuiLoraName(item?.name) : String(item?.name || "").trim();
      return {
        name,
    ...
    .filter((item) => !item.name || loraOptionValues.has(item.name))
...
function ensureComfyuiTemplateModelSelections(detail) {
  if (!detail?.ui_schema?.panels) return true;
  const missing = [];
  const ctx = { textFieldIndex: 0, loadImageIndex: 0 };
  comfyuiTemplateAllFields(detail).forEach((field) => {
    if (!field || field.synthetic || field.input_type === "embedding_shortcuts" || !field.node_id || !field.input_name) return;
    ...
    const binding = comfyuiTemplateFieldBinding(field, detail, ctx);
    if (binding.kind !== "model_select") return;
    const options = comfyuiTemplateModelSelectOptions(field);
    const select = $(`tmpl-${field.id || ""}`);
    const selected = select
      ? String(select.value || "").trim()
      : comfyuiTemplateModelSelectCurrentValue(field, options);
    const label = comfyuiTemplateFieldLabel(field, binding) || comfyuiTemplateModelOptionKindLabel(binding.optionKind);
    if (!options.length) {
      missing.push({ field, reason: `沒有可用的 ${label} 候選`, select });
    } else if (!selected) {
      missing.push({ field, reason: `尚未選擇 ${label}`, select });
    } else if (!options.some((option) => option.value === selected && !option.disabled)) {
      missing.push({ field, reason: `${label} 不在目前 ComfyUI 候選清單中`, select });
    }
  });
  if (!missing.length) return true;
  const labels = missing.map((item) => item.reason).slice(0, 4).join("、");
  setComfyuiMessage(`這個 workflow 的模型欄位尚未完成：${labels}。請先安裝或同步模型、重新整理模型清單後，從候選下拉選擇。`, false);
  const firstSelect = missing.find((item) => item.select)?.select;
...
async function ensureComfyuiTemplateImageAssignments(detail, operation = null) {
  const operationContext = operation || comfyuiOperationContext();
  comfyuiAssertOperationCurrent(operationContext);
  ...
  for (const assetKey of localAssetKeys) {
    await importer(assetKey, operationContext);
    comfyuiAssertOperationCurrent(operationContext);
  }
  renderSelectedComfyuiTemplate();
...
function comfyuiTemplateEmbeddingInsertionTarget(localTargets = []) {
  const active = document.activeElement;
  if (localTargets.includes(active)) return active;
  if (comfyuiTemplateLastFocusedTextFieldId) {
    const remembered = localTargets.find((el) => String(el.id || "") === comfyuiTemplateLastFocusedTextFieldId);
    if (remembered) return remembered;
  }
  return localTargets[0] || null;
}
...
function insertComfyuiTemplateEmbeddingToken(name, targetFieldIds = []) {
  ...
  const target = comfyuiTemplateEmbeddingInsertionTarget(localTargets);
  if (!target) return;
...
function renderComfyuiTemplateRunCountControl() {
  const value = typeof comfyuiRunCount === "function"
    ? comfyuiRunCount()
    : Math.max(1, Math.min(10, Number($("comfyui-run-count")?.value || 1) || 1));
  return `
    <div class="comfyui-template-run-count-control">
      <div>
        <div class="drive-card-title">生成張數</div>
        <div class="drive-card-sub">這是連續送出幾次產圖工作，不是 batch size。</div>
      </div>
      <input type="number" min="1" max="10" step="1" value="${sanitize(String(value))}" data-comfyui-template-run-count="1" aria-label="生成張數" />
    </div>
  `;
}
...
function renderComfyuiSdxlRefinerControl(detail = comfyuiSelectedTemplateDetail) {
  if (!comfyuiTemplateIsSdxlRefinerWorkflow(detail)) return "";
  return `
    <section class="comfyui-upscale-breakpoint-card comfyui-sdxl-refiner-card">
      <div>
        <div class="drive-card-title">SDXL Refiner</div>
        <div class="drive-card-sub">預設會使用 base + refiner 兩段去噪；跳過時系統會保留 base 的正負提示詞，改成 base 單段完整去噪、使用 base checkpoint 的 VAE，並只移除 refiner 那一路的模型與提示詞欄位。</div>
      </div>
      <label class="comfyui-template-checkbox">
        <input type="checkbox" data-comfyui-sdxl-skip-refiner="1"${comfyuiTemplateSdxlSkipRefiner ? " checked" : ""} />
        <span>跳過 Refiner，只用 SDXL base 完整去噪</span>
      </label>
    </section>
  `;
}

function renderComfyuiGgufWorkflowControl(detail = comfyuiSelectedTemplateDetail) {
  const state = ensureComfyuiGgufWorkflowState(detail);
  if (!state) return "";
  const profiles = comfyuiEnabledGgufProfiles();
  if (!profiles.length) {
    return `
      <section class="comfyui-upscale-breakpoint-card comfyui-gguf-profile-card">
        <div>
          <div class="drive-card-title">官方 GGUF profile</div>
          <div class="drive-card-sub">目前後端沒有回傳可用官方 GGUF profile；請先在管理設定檢查 ComfyUI / GGUF 資訊。</div>
        </div>
      </section>
    `;
  }
  const profile = comfyuiGgufProfileById(state.profileId) || profiles[0] || null;
  const variants = comfyuiEnabledGgufVariants(profile);
  const variant = comfyuiGgufVariantById(profile, state.variantId) || variants[0] || null;
  const profileHintParts = [];
  if (profile?.prompt_style_hint) profileHintParts.push(String(profile.prompt_style_hint));
  if (profile?.repo_id) profileHintParts.push(`repo：${profile.repo_id}`);
  if (variant?.gguf_file) profileHintParts.push(`GGUF：${variant.gguf_file}`);
  profileHintParts.push("送出時會自動套用 UNet、CLIP、VAE 與建議 sampler；若遠端缺模型或 ComfyUI-GGUF 節點，依賴檢查會列出需補項目。");
  return `
    <section class="comfyui-upscale-breakpoint-card comfyui-gguf-profile-card">
      <div>
        <div class="drive-card-title">官方 GGUF profile</div>
        <div class="drive-card-sub">把 GGUF 當作這個 workflow 的模板參數；提示詞、尺寸、seed 仍使用下方欄位。</div>
      </div>
      <div class="comfyui-multi-compare-row">
        <label>GGUF profile</label>
        <select data-comfyui-template-gguf-profile="1">
          ${profiles.map((item) => `<option value="${sanitize(item.id || "")}"${String(item.id || "") === String(profile?.id || "") ? " selected" : ""}>${sanitize(item.label || item.id || "GGUF profile")}</option>`).join("")}
        </select>
      </div>
      <div class="comfyui-multi-compare-row">
        <label>精度版本</label>
        <select data-comfyui-template-gguf-variant="1"${variants.length ? "" : " disabled=\"disabled\""}>
          ${variants.length ? variants.map((item) => `<option value="${sanitize(item.id || "")}"${String(item.id || "") === String(variant?.id || "") ? " selected" : ""}>${sanitize(typeof comfyuiGgufVariantLabel === "function" ? comfyuiGgufVariantLabel(item) : (item.label || item.id || "variant"))}</option>`).join("") : '<option value="">沒有可用精度</option>'}
        </select>
      </div>
      <div class="comfyui-template-direct-hint">${sanitize(profileHintParts.join(" "))}</div>
    </section>
  `;
}
...
function renderComfyuiMultiCompareControl(detail = comfyuiSelectedTemplateDetail) {
  const state = ensureComfyuiMultiCompareState(detail);
  if (!state) return "";
  const modelOptions = comfyuiMultiCompareCheckpointOptions(detail);
  const loraOptions = comfyuiTemplateLoraSelectOptions({});
  const maxLoras = comfyuiMultiCompareMaxLoras();
  const checkpointOptionHtml = (checkpoint) => (
    modelOptions.length
      ? modelOptions.map((option) => `<option value="${sanitize(option.value)}"${option.value === checkpoint ? " selected" : ""}>${sanitize(option.label)}</option>`).join("")
      : '<option value="" selected disabled="disabled">目前沒有可用大模型</option>'
  );
  const checkpointRows = state.checkpoints.map((checkpoint, index) => `
    <div class="comfyui-multi-compare-row">
      <label>大模型 #${index + 1}</label>
      <select data-comfyui-multi-compare-checkpoint="${index}"${modelOptions.length ? "" : ' disabled="disabled"'}>
        ${checkpointOptionHtml(checkpoint)}
      </select>
    ...
    <section class="comfyui-multi-compare-card">
      <div class="comfyui-multi-compare-head">
        ...
        <div class="drive-file-actions">
          <button class="btn btn-sm" type="button" data-comfyui-multi-compare-add-checkpoint="1"${state.checkpoints.length >= COMFYUI_MULTI_COMPARE_MAX_CHECKPOINTS || !modelOptions.length ? ' disabled="disabled"' : ""}>新增大模型</button>
...
function renderComfyuiTemplateEmbeddingShortcuts(field, detail = comfyuiSelectedTemplateDetail) {
  if (!comfyuiTemplateSupportsEmbeddingShortcuts(detail)) return "";
...
function renderComfyuiTemplateField(field, detail, ctx) {
  ...
  if (binding.kind === "readonly") {
    const editAction = binding.editableLockedModel
      ? `
        <div class="drive-file-actions" style="justify-content:flex-start;margin-top:.45rem;">
          <button class="btn btn-sm" type="button" data-comfyui-template-model-edit="${sanitize(field.id || "")}">選擇可用模型</button>
        </div>
        <div class="comfyui-template-direct-hint">保留模板預設可維持官方建議；若目前 ComfyUI 沒有該模型，請改從已列出的同類模型候選中選擇。</div>
      `
      : "";
    return `
      <div class="${cardClass}">
  ...
  if (binding.kind === "model_select") {
    const options = comfyuiTemplateModelSelectOptions(field);
    const current = comfyuiTemplateModelSelectCurrentValue(field, options);
    const hint = comfyuiTemplateModelSelectHint(field, options);
    const filterActions = comfyuiTemplateModelSelectFilterActions(field);
    const emptyLabel = options.length
      ? `請選擇${comfyuiTemplateModelOptionKindLabel(binding.optionKind)}`
      : `目前沒有可用的${comfyuiTemplateModelOptionKindLabel(binding.optionKind)}`;
    ...
    return `
      <div class="${cardClass}">
        ...
        <select id="tmpl-${sanitize(field.id || "")}" data-comfyui-template-model-field="${sanitize(field.id || "")}" data-comfyui-template-label="${sanitize(fieldLabel)}">
          <option value=""${current ? "" : " selected"}${options.length ? "" : ' disabled="disabled"'}>${sanitize(emptyLabel)}</option>
          ${options.map((option) => `<option value="${sanitize(option.value)}"${option.value === current ? " selected" : ""}>${sanitize(option.label)}</option>`).join("")}
        </select>
  ...
  if (binding.kind === "lora") {
    ...
    const currentAvailable = options.some((option) => option.value === current && !option.disabled);
    const selectedValue = currentAvailable ? current : "";
    const missingHint = current && !currentAvailable
      ? `<div class="comfyui-template-direct-hint">模板預設 LoRA ${sanitize(current)} 不在目前 ComfyUI LoRA 候選清單中；已預設不使用，請從候選下拉選擇。</div>`
      : "";
    return `
      <div class="${cardClass}">
        ...
        <select id="tmpl-${sanitize(field.id || "")}" data-comfyui-template-lora-node="${sanitize(binding.nodeId)}">
          ${options.map((option) => `<option value="${sanitize(option.value)}"${option.value === selectedValue ? " selected" : ""}${option.disabled ? ' disabled="disabled"' : ""}>${sanitize(option.label)}</option>`).join("")}
        </select>
...
function bindRenderedComfyuiTemplateFields(detail) {
    ...
    if (el.tagName === "TEXTAREA" || el.tagName === "INPUT") {
      ["focus", "click", "keyup", "select", "mouseup", "touchend"].forEach((eventName) => {
        el.addEventListener(eventName, () => {
          comfyuiTemplateLastFocusedTextFieldId = String(el.id || "");
        });
      });
    }
  });
  ...
  host.querySelectorAll("[data-comfyui-template-model-field]").forEach((el) => {
    if (el.dataset.boundComfyuiTemplate === "1") return;
    el.dataset.boundComfyuiTemplate = "1";
    const fieldId = el.getAttribute("data-comfyui-template-model-field") || "";
    const sync = () => {
  ...
  host.querySelectorAll("[data-comfyui-template-model-filter]").forEach((button) => {
    if (button.dataset.boundComfyuiTemplate === "1") return;
    button.dataset.boundComfyuiTemplate = "1";
    button.addEventListener("click", () => {
      const fieldId = button.getAttribute("data-comfyui-template-model-filter") || "";
      const mode = button.getAttribute("data-comfyui-template-model-filter-mode") || "all";
      if (!fieldId) return;
      if (mode === "all") comfyuiTemplateModelShowAllFields[fieldId] = true;
      else delete comfyuiTemplateModelShowAllFields[fieldId];
      renderSelectedComfyuiTemplate({ preserveOpenPanels: true });
    });
  });
  ...
  host.querySelectorAll("[data-comfyui-template-run-count]").forEach((input) => {
    if (input.dataset.boundComfyuiTemplate === "1") return;
    input.dataset.boundComfyuiTemplate = "1";
    const sync = () => {
      const value = Math.max(1, Math.min(10, Math.floor(Number(input.value || 1) || 1)));
      input.value = String(value);
      const globalInput = $("comfyui-run-count");
      if (globalInput) globalInput.value = String(value);
      writeComfyuiDraft();
    };
    input.addEventListener("input", sync);
    input.addEventListener("change", sync);
  });
  host.querySelectorAll("[data-comfyui-sdxl-skip-refiner]").forEach((input) => {
    if (input.dataset.boundComfyuiTemplate === "1") return;
    input.dataset.boundComfyuiTemplate = "1";
    input.addEventListener("change", () => {
      comfyuiTemplateSdxlSkipRefiner = !!input.checked;
      writeComfyuiDraft();
      renderSelectedComfyuiTemplate({ preserveOpenPanels: true });
    });
  });
  host.querySelectorAll("[data-comfyui-template-gguf-profile]").forEach((select) => {
    if (select.dataset.boundComfyuiTemplate === "1") return;
    select.dataset.boundComfyuiTemplate = "1";
    select.addEventListener("change", () => {
      const state = ensureComfyuiGgufWorkflowState(detail);
      if (!state) return;
      const profile = comfyuiGgufProfileById(select.value);
      const firstVariant = comfyuiEnabledGgufVariants(profile)[0] || null;
      state.profileId = profile?.id || "";
      state.variantId = firstVariant?.id || "";
      applyComfyuiGgufWorkflowProfileDefaults(detail, { overwrite: true });
      writeComfyuiDraft();
      renderSelectedComfyuiTemplate({ preserveOpenPanels: true });
    });
  });
  host.querySelectorAll("[data-comfyui-template-gguf-variant]").forEach((select) => {
    if (select.dataset.boundComfyuiTemplate === "1") return;
    select.dataset.boundComfyuiTemplate = "1";
    select.addEventListener("change", () => {
      const state = ensureComfyuiGgufWorkflowState(detail);
      if (!state) return;
      state.variantId = select.value || "";
      writeComfyuiDraft();
    });
  });
  ...
  host.querySelectorAll("[data-comfyui-multi-compare-add-checkpoint]").forEach((button) => {
    ...
    button.addEventListener("click", () => {
      ...
      const optionValues = comfyuiMultiCompareCheckpointOptions(detail).map((option) => option.value).filter(Boolean);
      if (!optionValues.length) {
        setComfyuiMessage("目前沒有可用大模型候選，請先安裝或同步 checkpoint 後重新整理模型清單。", false);
        return;
      }
      const fallback = optionValues.includes(state.checkpoints[state.checkpoints.length - 1])
  ...
  host.querySelectorAll("[data-comfyui-template-image-picker]").forEach((button) => {
    ...
    button.addEventListener("click", () => {
      openComfyuiImagePicker(button.getAttribute("data-comfyui-template-image-picker"))
        .catch((err) => handleComfyuiActionError(err, "圖片選擇器開啟失敗"));
    });
  });
...
function renderSelectedComfyuiTemplate({ preserveOpenPanels = false } = {}) {
  ...
  const openPanelIds = preserveOpenPanels
    ...
    : new Set();
  const runCountHtml = renderComfyuiTemplateRunCountControl();
  const promptSharingHtml = renderComfyuiTemplatePromptSharingControl(detail);
  const sdxlRefinerHtml = renderComfyuiSdxlRefinerControl(detail);
  const ggufWorkflowHtml = renderComfyuiGgufWorkflowControl(detail);
  ...
  host.innerHTML = runCountHtml + promptSharingHtml + sdxlRefinerHtml + ggufWorkflowHtml + multiCompareHtml + upscaleBreakpointHtml + panels.map((panel) => {
...
function renderComfyuiWorkflowPresetList(targetId, items, emptyText) {
    ...
    return `
      <details class="comfyui-workflow-item">
        ...
        <div class="drive-file-actions" style="justify-content:flex-start;margin-top:.55rem;">
          <button class="btn btn-sm" type="button" data-comfyui-workflow-apply="${item.id}">套用模板預設</button>
  ...
  list.querySelectorAll("[data-comfyui-workflow-run-apply]").forEach((button) => {
    button.addEventListener("click", () => {
      applyComfyuiWorkflowRunToForm(Number(button.getAttribute("data-comfyui-workflow-run-apply")))
        .catch((err) => handleComfyuiActionError(err, "workflow 執行紀錄套回失敗"));
    });
  });
  list.querySelectorAll("[data-comfyui-workflow-run]").forEach((button) => {
    button.addEventListener("click", () => {
      runComfyuiWorkflowPreset(Number(button.getAttribute("data-comfyui-workflow-run"))).catch((err) => {
        if (err?.name !== "AbortError") setComfyuiMessage(err.message || "workflow 執行失敗", false);
      });
    });
  });
  list.querySelectorAll("[data-comfyui-workflow-export]").forEach((button) => {
    button.addEventListener("click", () => {
      exportComfyuiWorkflowPreset(Number(button.getAttribute("data-comfyui-workflow-export"))).catch((err) => handleComfyuiActionError(err, "workflow 匯出失敗"));
    });
  });
  list.querySelectorAll("[data-comfyui-workflow-edit]").forEach((button) => {
    button.addEventListener("click", () => {
      loadComfyuiWorkflowPresetIntoEditor(Number(button.getAttribute("data-comfyui-workflow-edit"))).catch((err) => {
        if (err?.name !== "AbortError") setComfyuiMessage(err.message || "workflow 讀取失敗", false);
      });
    });
  });
  list.querySelectorAll("[data-comfyui-workflow-duplicate]").forEach((button) => {
    button.addEventListener("click", () => {
      duplicateComfyuiWorkflowPreset(Number(button.getAttribute("data-comfyui-workflow-duplicate"))).catch((err) => {
        if (err?.name !== "AbortError") setComfyuiMessage(err.message || "workflow 複製失敗", false);
      });
    });
  });
  list.querySelectorAll("[data-comfyui-workflow-default]").forEach((button) => {
    button.addEventListener("click", () => {
      setDefaultComfyuiWorkflowPreset(Number(button.getAttribute("data-comfyui-workflow-default"))).catch((err) => {
        if (err?.name !== "AbortError") setComfyuiMessage(err.message || "預設版面設定失敗", false);
      });
    });
  });
  list.querySelectorAll("[data-comfyui-workflow-publish]").forEach((button) => {
    button.addEventListener("click", () => {
      publishComfyuiWorkflowPresetOfficial(Number(button.getAttribute("data-comfyui-workflow-publish"))).catch((err) => {
        if (err?.name !== "AbortError") setComfyuiMessage(err.message || "官方 preset 發布失敗", false);
      });
    });
  });
  list.querySelectorAll("[data-comfyui-workflow-delete]").forEach((button) => {
    button.addEventListener("click", () => {
      deleteComfyuiWorkflowPreset(Number(button.getAttribute("data-comfyui-workflow-delete"))).catch((err) => {
        if (err?.name !== "AbortError") setComfyuiMessage(err.message || "workflow 刪除失敗", false);
      });
    });
  });
}
...
async function loadComfyuiWorkflowPresets(options = {}) {
  const { silentTemplateReload = true } = options || {};
  const operation = options?.operation || comfyuiOperationContext();
  const loadGeneration = ++comfyuiWorkflowPresetsLoadGeneration;
  comfyuiAssertOperationCurrent(operation);
  ...
  const json = await res.json().catch(() => ({}));
  comfyuiAssertOperationCurrent(operation);
  if (loadGeneration !== comfyuiWorkflowPresetsLoadGeneration) throw comfyuiAccountAbortError();
  if (!res.ok || !json.ok) {
...
function applyComfyuiWorkflowPresetDefaults(defaults = {}) {
  ...
  if (typeof setComfyuiSamplingFieldsFromValues === "function") {
...
async function loadComfyuiWorkflowPresetIntoEditor(presetId) {
  const operation = comfyuiOperationContext();
  const loadGeneration = ++comfyuiWorkflowEditorLoadGeneration;
  await fetchCsrfToken({ force: true });
  comfyuiAssertOperationCurrent(operation);
  const res = await apiFetch(API + `/comfyui/workflows/${encodeURIComponent(presetId)}`, {
    credentials: "same-origin",
  ...
  const json = await res.json().catch(() => ({}));
  comfyuiAssertOperationCurrent(operation);
  if (loadGeneration !== comfyuiWorkflowEditorLoadGeneration) throw comfyuiAccountAbortError();
...
async function runComfyuiWorkflowPreset(presetId, options = {}) {
  const operation = options.operation || comfyuiOperationContext();
  comfyuiAssertOperationCurrent(operation);
  if (!presetId) return;
  ...
  if (templateDetail && !ensureComfyuiTemplateModelSelections(templateDetail)) return;
  ...
  try {
    imageAssignmentState = templateDetail ? await ensureComfyuiTemplateImageAssignments(templateDetail, operation) : imageAssignmentState;
    if (options.runToken) comfyuiAssertGenerationCurrent(operation, options.runToken);
    else comfyuiAssertOperationCurrent(operation);
  } catch (err) {
    if (!comfyuiOperationIsCurrent(operation) || err?.name === "AbortError") throw err;
  ...
  const upscaleBreakpointSpec = templateDetail && comfyuiTemplateIsMultiMethodUpscale(templateDetail)
    ...
    : null;
  const sdxlRefinerSpec = templateDetail && comfyuiTemplateIsSdxlRefinerWorkflow(templateDetail)
    ? comfyuiSdxlRefinerRunSpec(templateDetail)
    : null;
  const ggufWorkflowSpec = templateDetail && comfyuiTemplateIsGgufWorkflow(templateDetail)
    ? comfyuiGgufWorkflowRunSpec(templateDetail)
    : null;
  ...
  const inheritedRunToken = options.runToken || null;
  const runToken = inheritedRunToken || {};
  const ownsRunToken = !inheritedRunToken;
  if (ownsRunToken && comfyuiGenerateAbortController) comfyuiGenerateAbortController.abort();
  comfyuiGenerationRunToken = runToken;
  await fetchCsrfToken({ force: true });
  comfyuiAssertGenerationCurrent(operation, runToken);
  ...
  let selectedVae = String($("comfyui-vae-select")?.value || "").trim();
  const templateVae = comfyuiTemplateSyntheticVaeFieldValue(templateDetail);
  if (templateVae) {
    selectedVae = templateVae;
  } else if (selectedVae === COMFYUI_VAE_BUILTIN || !selectedVae) {
    selectedVae = "";
  }
  ...
  const runRequest = (confirmed, requestUserInputs) => apiFetch(API + `/comfyui/workflows/${encodeURIComponent(presetId)}/run`, {
    ...
    body: JSON.stringify({
      ...
      vae: selectedVae && selectedVae !== COMFYUI_VAE_BUILTIN ? selectedVae : "",
    }),
  });
  ...
  const renderPartialWorkflowResult = async (partialResult) => {
    ...
    const images = await hydrateComfyuiGeneratedImages(rawImages, operation);
    comfyuiAssertGenerationCurrent(operation, runToken);
  ...
  try {
    ...
    for (let runIndex = 0; runIndex < runCount; runIndex += 1) {
      ...
      const result = await pollComfyuiJobUntilDone(jobId, controller, workflowTimeoutSeconds, {
        ...
        onPartialError: (err) => {
          if (err?.name !== "AbortError") console.warn("Multi-Compare partial preview failed", err);
        },
      });
      comfyuiAssertGenerationCurrent(operation, runToken);
      ...
      const images = await hydrateComfyuiGeneratedImages(rawImages, operation);
      comfyuiAssertGenerationCurrent(operation, runToken);
      const media = await hydrateComfyuiGeneratedMedia(Array.isArray(result.media) ? result.media : [], jobId, operation);
      comfyuiAssertGenerationCurrent(operation, runToken);
    ...
    await loadComfyuiWorkflowPresets({ operation });
    comfyuiAssertGenerationCurrent(operation, runToken);
    if (typeof loadComfyuiHistory === "function") loadComfyuiHistory(operation).catch((err) => {
      if (comfyuiOperationIsCurrent(operation) && comfyuiGenerationRunToken === runToken && err?.name !== "AbortError") {
        setComfyuiHistoryActionMessage(err?.message || "ComfyUI 歷史重新整理失敗", false);
      }
    });
    ...
    if (!comfyuiOperationIsCurrent(operation) || comfyuiGenerationRunToken !== runToken) return;
    ...
    if (comfyuiOperationIsCurrent(operation) && comfyuiGenerationRunToken === runToken) {
      if (comfyuiGenerateAbortController === controller) comfyuiGenerateAbortController = null;
      setComfyuiBusy(false);
      if (ownsRunToken) comfyuiGenerationRunToken = null;
    }
  }
}
...
async function loadComfyuiWorkflowFile() {
  const input = $("comfyui-workflow-file");
  const file = input?.files?.[0] || null;
  if (!file) return;
  const accountGeneration = comfyuiWorkflowAccountGeneration;
  const editorLoadGeneration = ++comfyuiWorkflowEditorLoadGeneration;
  const text = await file.text();
  if (
    accountGeneration !== comfyuiWorkflowAccountGeneration
    || editorLoadGeneration !== comfyuiWorkflowEditorLoadGeneration
    || input !== $("comfyui-workflow-file")
    || input?.files?.[0] !== file
  ) return;
