...
class EffectiveLoadWindow:
    ...
    def evidence(self) -> dict[str, Any]:
        ...
        target_scheduled = self.scheduled_load_level == 32
        enough_workers = self.active_workers >= 28
        ...
        if not self.maintenance_window:
            if not target_scheduled:
                failures.append("SCHEDULED_LOAD_NOT_32")
            if not enough_workers:
                failures.append("ACTIVE_WORKERS_BELOW_28")
        ...
        return {
            ...
            "target_conditions": {
                "scheduled_load_level_32": target_scheduled,
                "active_workers_at_least_28": enough_workers,
    ...
    *,
    ...
    for index, row in enumerate(rows):
        ...
        if row.get("maintenance_window"):
            ...
            continue
        # Ramp windows below 32 are intentionally outside the post-ramp target
        # denominator.  A later unexpected drop below 32 must carry a
        # degradation reason and therefore remains eligible/failing.
        if int(row.get("scheduled_load_level") or 0) < 32 and not row.get("degradation_reason"):
            continue
