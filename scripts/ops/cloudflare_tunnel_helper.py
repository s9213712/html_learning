#!/usr/bin/env python3
"""Small helper for Cloudflare Tunnel dev workflows.
...
CLOUDFLARE_GPG_URL = "https://pkg.cloudflare.com/cloudflare-main.gpg"
CLOUDFLARE_APT_SOURCE = (
    "deb [signed-by=/usr/share/keyrings/cloudflare-main.gpg] "
    "https://pkg.cloudflare.com/cloudflared any main\n"
)
CLOUDFLARE_APT_KEYRING = Path("/usr/share/keyrings/cloudflare-main.gpg")
CLOUDFLARE_APT_LIST = Path("/etc/apt/sources.list.d/cloudflared.list")
...
def sudo_prefix() -> list[str]:
    if IS_WINDOWS:
        raise RuntimeError("apt install is only supported on Linux")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        return []
    sudo = shutil.which("sudo")
    if not sudo:
        raise RuntimeError("sudo is required to install cloudflared with apt")
    return [sudo]


def run_command(cmd: list[str], *, input_bytes: bytes | None = None, timeout: int | None = None) -> str:
...
def fetch(url: str, timeout: int = 15, *, insecure: bool = False) -> tuple[int | None, str]:
    if shutil.which("curl"):
        base_args = ("-k",) if insecure else ()
        result = run_curl(url, timeout, base_args)
        if result.returncode == 6:
            ...
            doh_result = run_curl(url, timeout, (*base_args, "--doh-url", DOH_URL))
    ...
    req = Request(url, headers={"User-Agent": "hackme-web-cloudflare-tunnel/1.0"})
    try:
...
def verify_local(url: str, *, insecure: bool = False) -> None:
    status, _ = fetch(url, timeout=10, insecure=insecure)
    if status is None:
        return
...
def cmd_install(args: argparse.Namespace) -> int:
    try:
        if args.method != "apt":
            raise RuntimeError(f"unsupported install method: {args.method}")
        commands = [
            f"curl -fsSL {CLOUDFLARE_GPG_URL} | sudo gpg --dearmor -o {CLOUDFLARE_APT_KEYRING}",
            f"echo {json.dumps(CLOUDFLARE_APT_SOURCE.strip())} | sudo tee {CLOUDFLARE_APT_LIST}",
            "sudo apt-get update",
            "sudo apt-get install -y cloudflared" if args.yes else "sudo apt-get install cloudflared",
        ]
        if args.dry_run:
            print_json({"ok": True, "dry_run": True, "method": "apt", "commands": commands})
            return 0

        existing = shutil.which("cloudflared")
        if existing and not args.force:
            result = subprocess.run([existing, "--version"], text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            print_json(
                {
                    "ok": True,
                    "installed": False,
        ...
        if platform.system().lower() != "linux":
            raise RuntimeError("apt install is only supported on Linux")
        for name in ("gpg", "apt-get"):
            if not shutil.which(name):
                raise RuntimeError(f"missing required command: {name}")

        prefix = sudo_prefix()
        with urlopen(CLOUDFLARE_GPG_URL, timeout=args.timeout) as resp:
            key_bytes = resp.read()
        run_command([*prefix, "gpg", "--batch", "--yes", "--dearmor", "-o", str(CLOUDFLARE_APT_KEYRING)], input_bytes=key_bytes)
        run_command([*prefix, "tee", str(CLOUDFLARE_APT_LIST)], input_bytes=CLOUDFLARE_APT_SOURCE.encode("utf-8"))
        run_command([*prefix, "apt-get", "update"], timeout=max(args.timeout, 60))
        install_cmd = [*prefix, "apt-get", "install"]
        if args.yes:
            install_cmd.append("-y")
        install_cmd.append("cloudflared")
        install_output = run_command(install_cmd, timeout=max(args.timeout, 120))

        path = require_cloudflared()
        version = subprocess.run([path, "--version"], text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        print_json(
            {
                "ok": True,
                "installed": True,
...
def cmd_quick(args: argparse.Namespace) -> int:
    try:
        ...
        verify_local(local_url, insecure=bool(args.no_tls_verify))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    install = sub.add_parser("install", help="install cloudflared")
    install.add_argument("--method", default="apt", choices=["apt"], help="install method")
    install.add_argument("--force", action="store_true", help="reinstall even if cloudflared is already in PATH")
    install.add_argument("--yes", dest="yes", action="store_true", default=True, help="pass -y to apt-get install")
    install.add_argument("--no-yes", dest="yes", action="store_false", help="do not pass -y to apt-get install")
    install.add_argument("--dry-run", action="store_true", help="print install commands without changing the system")
    install.add_argument("--timeout", type=int, default=120, help="network/apt command timeout seconds")
